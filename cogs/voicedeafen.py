...
class VoiceDeafen(commands.Cog):
    ...
    async def mutecasque(
        ...
        if member.voice.deaf:
            await ctx.send(embed=embeds.warn(
                t(ctx, "voice.already_deafened", user=member.mention)))
            return
        try:
        ...
        await ctx.send(embed=embeds.success(
            t(ctx, "voice.deafened", user=member.mention)))
    ...
    async def unmutecasque(
        ...
        if not member.voice.deaf:
            await ctx.send(embed=embeds.warn(
                t(ctx, "voice.not_deafened", user=member.mention)))
            return
        try:
        ...
        await ctx.send(embed=embeds.success(
            t(ctx, "voice.undeafened", user=member.mention)))
