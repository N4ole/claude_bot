...
class VoiceMute(commands.Cog):
    ...
    async def mutemicro(
        ...
        if member.voice.mute:
            await ctx.send(t(ctx, "voice.already_mic_muted", user=member.mention))
            return
        try:
        ...
        await ctx.send(t(ctx, "voice.mic_muted", user=member.mention))
    ...
    async def unmutemicro(
        ...
        if not member.voice.mute:
            await ctx.send(t(ctx, "voice.not_mic_muted", user=member.mention))
            return
        try:
        ...
        await ctx.send(t(ctx, "voice.mic_unmuted", user=member.mention))
